...
Lemma in_State_step : forall M ds st, (0 < M)%Z -> Z.Odd (divsteps.f ds) ->
 in_State M ds st -> in_State M (divsteps.step ds) (processDivstep M st).
Proof.
...
assert (HM2 : 0 < M).
 ...
 rewrite <- Zlt_Qlt.
 assumption.
cut (in_State M (N.iter N divsteps.step (divsteps.init f g)) (N.iter N (processDivstep M) state0)).
 intros [H0|H0];auto.
 set (p := (_,_)) in *.
